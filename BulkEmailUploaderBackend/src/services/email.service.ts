...
console.log("envs", process.env.SENDGRID_API_KEY, process.env.SENDGRID_FROM_EMAIL);
...
export class EmailService {
  ...
  static async sendWelcomeEmail(user: IUser): Promise<boolean> {
    try {
      ...
      if (!emailRegex.test(process.env.SENDGRID_FROM_EMAIL)) {
        throw new Error(`Invalid FROM email format: ${process.env.SENDGRID_FROM_EMAIL}`);
      }
  ...
  static async sendBulkEmailsWithTemplate(users: IUser[]): Promise<{ success: number; failure: number }> {
    try {
      ...
      console.log(`Attempting to send ${messages.length} emails from: ${process.env.SENDGRID_FROM_EMAIL}`);

      // Send emails in batch (up to ~1000 per request)
      await sgMail.send(messages);

      console.log(`Bulk email sending successful for ${users.length} users`);
      ...
      WebSocketService.broadcastCompletion(users.length, 0, users.length);
      return { success: users.length, failure: 0 };
      ...
      WebSocketService.broadcastCompletion(0, users.length, users.length);
      return { success: 0, failure: users.length };
    }
  }
}