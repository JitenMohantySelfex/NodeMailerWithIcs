...
//   timeout: 60000, // Increased global timeout for navigation
// };
...
//       'a.sc-db8c6f48-0.egXQHq.sc-b15be207-1.esilwF' // More specific class-based selector
//     ];
...
//     console.log("🔍 Looking for 'See All' buttons for categories within the store...");
//     const seeAllButtonSelectors = [
//       'a[data-anchor-id="CarouselSeeAllButton"]',
//       'button[aria-label="See All"]',
//       'span.sc-aXZVg.hpVWFC:has-text("See All")', // More specific text match
//       'a[href*="/collection?store_id="]', // Generic link for collection
//     ];

//     let seeAllClicked = false;
//     for (const selector of seeAllButtonSelectors) {
//       const seeAllButton = await page.$(selector);
//       if (seeAllButton) {
//         console.log(`✅ Found 'See All' button with selector: ${selector}, clicking...`);
//         seeAllClicked = await clickWithWait(page, selector, { waitTime: 3000, timeout: 20000 }); // Increased timeout
//         if (seeAllClicked) {
//           break;
//         }
//       }
...
//     if (!seeAllClicked) {
//       console.log("⚠️ No 'See All' button found or clicked for this store. Skipping product scraping for this store.");
//       return; // Skip if no "See All" button is found/clicked
//     }

//     await waitFor(5000); // Wait for the collection page to load
//     await takeScreenshot(page, `store-${store.storeId || 'unknown'}-collection.png`);

//     console.log("🔄 Auto-scrolling to load all products...");
//     await autoScroll(page);
//     await waitFor(3000); // Give a moment after scrolling

//     console.log("⛏️ Scraping product details...");
//     const products = await page.evaluate((storeData) => { // Pass storeData as an argument
...
//         // Attempt to get current price (can be split into dollar and cents)
...
//         if (currentPriceMain !== '') { // Check against empty string not 'N/A' as it's an initial value
//             currentPrice = `${currentPriceSymbol}${currentPriceMain}${currentPriceDecimal}`;
//         } else {
//             // Fallback for current price if combined span is used
...
//         return {
//           storeName: storeData.text, // Use storeData here
//           storeUrl: storeData.href,   // Use storeData here
//           storeId: storeData.storeId, // Include storeId in product data
//           name,
//           currentPrice,
//           originalPrice,
//           discount,
...
//         };
//       });
//     }, store); // Pass store object to evaluate context

//     console.log(`✅ Found ${products.length} products for ${store.text || store.href}`);
