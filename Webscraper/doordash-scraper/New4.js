  ...
  await page.evaluate(async () => {
    await new Promise((resolve) => {
      ...
      const timer = setInterval(() => {
        ...
        const currentCount = document.querySelectorAll('[data-testid="RetailItemCardPriceNameInfoBadgingFramework"]').length;
        ...
        if ((totalHeight >= scrollHeight && window.noNewProductsCount >= 5) || 
...
async function addAddress(page, address) {
  ...
  try {
    ...
    // Cloudflare verification handling is removed as per user request.
    // User will handle this manually.

    await handleBotVerification(page); // Check for bot verification
    ...
    await closePopups(page);
    await handleBotVerification(page); // Check for bot verification after clicking address input
    ...
    await closePopups(page); // Close popups after pressing Enter
    await handleBotVerification(page); // Check for bot verification after pressing Enter
...
async function scrapeVerticalLinks(page, verticalName, targetUrl, scrapedDataKey, screenshotPrefix) {
  try {
    console.log(`🛒 Step: Ensuring we are on the ${verticalName} page or navigating to it...`);
    ...
    console.log(`🔍 Looking for 'See All' button for ${verticalName} categories...`);
    ...
    if (seeAllClicked) {
      console.log(`✅ Clicked 'See All' button for ${verticalName} categories. Waiting for store cards to load...`);
      ...
      await takeScreenshot(page, `${screenshotPrefix}-categories-see-all-page.png`);
      await handleBotVerification(page);
    } else {
      console.log(`⚠️ Could not click 'See All' button for ${verticalName} categories. Proceeding to scrape existing store links.`);
      await page.waitForSelector('[data-anchor-id="StoreCard"]', {
        timeout: CONFIG.timeout,
      });
      await waitFor(3000);
...
async function scrapeProductsWithRetry(page, storeData, collectionLink, storeName, collectionName) {
  let allProducts = [];
  let attempts = 0;
  const maxAttempts = 3;
  ...
  while (attempts < maxAttempts) {
    attempts++;
    console.log(`🔄 Attempt ${attempts}/${maxAttempts} to scrape all products...`);
    ...
    const currentProductCount = await page.evaluate(() => {
      return document.querySelectorAll('[data-testid="RetailItemCardPriceNameInfoBadgingFramework"], [data-testid*="StoreItemCard"]').length;
    });
    
    console.log(`📊 Found ${currentProductCount} products in attempt ${attempts}`);
    
    // Scrape products with multiple selector options
    const products = await page.evaluate((storeData, currentCollectionLink, currentStoreName, currentCollectionName) => {
      ...
      return productElements.map(el => {
        ...
        const name = el.querySelector('[data-telemetry-id="priceNameInfo.name"], [data-testid*="name"]')?.textContent.trim() || 'N/A';
        
        // Price extraction with fallbacks
        let currentPrice = 'N/A';
        const priceElement = el.querySelector('.sc-85923f71-0.hlWxbc, [data-testid*="price"], [class*="price"]');
        if (priceElement) {
          const main = priceElement.querySelector('.sc-85923f71-2.cGEDSS, [class*="integer"]')?.textContent.trim() || '';
          const decimal = priceElement.querySelector('.sc-85923f71-1.kQLTAj, [class*="decimal"]')?.textContent.trim() || '';
          const symbol = priceElement.querySelector('.sc-85923f71-1.erPBol, [class*="currency"]')?.textContent.trim() || '';
          currentPrice = `${symbol}${main}${decimal}` || priceElement.textContent.trim();
        }
        
        const originalPrice = el.querySelector('.sc-aXZVg.gQfTyJ, [data-testid*="originalPrice"]')?.textContent.trim() || 'N/A';
        const discount = el.querySelector('[data-testid*="percent_discount"] span, [class*="discount"]')?.textContent.trim() || 'N/A';
        const rating = el.querySelector('[data-testid*="rating"], [class*="rating"]')?.textContent.trim() || 'N/A';
        const ratingCount = el.querySelector('[data-testid*="ratingCount"], [class*="ratingCount"]')?.textContent.trim() || 'N/A';
    ...
    }, storeData, collectionLink, storeName, collectionName);
...
async function scrapeStoreProducts(page, store, productDataKey) {
  console.log(`🛍️ Navigating to store: ${store.text || store.href} (ID: ${store.storeId || 'N/A'})`);
  ...
  try {
    ...
    for (let i = 0; i < 3; i++) {
      try {
        ...
        console.warn(`⚠️ Navigation failed (attempt ${i + 1}/3): ${navError.message}`);
        await waitFor(5000);
      }
    }

    if (!navigationSuccess) {
      throw new Error(`Failed to navigate to store page after 3 retries: ${store.href}`);
    }
    ...
    if (collectionLinks.length === 0) {
      console.log("⚠️ No collection links found — scraping main store page directly...");
      const products = await scrapeProductsWithRetry(page, store, store.href, storeName, "Main Store Products");

      if (!scrapedData[productDataKey][storeName]["Main Store Products"]) {
        scrapedData[productDataKey][storeName]["Main Store Products"] = [];
      }

      scrapedData[productDataKey][storeName]["Main Store Products"].push(...products);
      await saveToJSON();
    } else {
      for (const collectionLink of collectionLinks) {
        ...
        try {
          ...
          const collectionName = collectionNameElement
            ? await page.evaluate(el => el.textContent.trim(), collectionNameElement)
            : 'Unnamed Collection';
          ...
          const products = await scrapeProductsWithRetry(page, store, collectionLink, storeName, collectionName);
          console.log(`✅ Collected ${products.length} products for: ${collectionName}`);

          scrapedData[productDataKey][storeName][collectionName].push(...products);
          await saveToJSON();
        } catch (collectionError) {
          console.error(`❌ Error scraping collection ${collectionLink}: ${collectionError.message}`);
    ...
    console.error(`❌ Failed to scrape store: ${store.text || store.href} (${store.storeId || 'N/A'}): ${error.message}`);
    await takeScreenshot(page, `store-${store.storeId || 'unknown'}-error.png`);
  }
}


async function saveToJSON(filename = "doordash_scraped_data.json") { // Changed filename for overall data
  try {
    ...
    for (const verticalKey in scrapedData) {
      if (verticalKey.includes("Products")) { // Only iterate through product data
    ...
    console.log(`📊 Total retail stores scraped: ${scrapedData.retailStores?.length || 0}`);
    console.log(`📊 Total convenience stores scraped: ${scrapedData.convenienceStores?.length || 0}`);
    console.log(`📊 Total alcohol stores scraped: ${scrapedData.alcoholStores?.length || 0}`);
    console.log(`📊 Total products scraped across all verticals: ${totalProductsCount}`);
  } catch (error) {
...
async function scrapeDoorDash() {
  ...
  try {
    ...
    const groceryStoreLinks = await scrapeVerticalLinks(page, "Grocery", "https://www.doordash.com/tabs/grocery", "groceryStores", "grocery");
    for (const store of groceryStoreLinks) {
      await scrapeStoreProducts(page, store, "groceryProducts");
    }

    // Scrape Retail
    console.log("\n--- Scraping Retail Stores ---");
    const retailStoreLinks = await scrapeVerticalLinks(page, "Retail", "https://www.doordash.com/tabs/retails", "retailStores", "retail");
    for (const store of retailStoreLinks) {
      await scrapeStoreProducts(page, store, "retailProducts");
    }

    // Scrape Convenience
    console.log("\n--- Scraping Convenience Stores ---");
    const convenienceStoreLinks = await scrapeVerticalLinks(page, "Convenience", "https://www.doordash.com/vertical_homepage?cursor=&vertical_id=2", "convenienceStores", "convenience");
    for (const store of convenienceStoreLinks) {
      await scrapeStoreProducts(page, store, "convenienceProducts");
    }

    // Scrape Alcohol
    console.log("\n--- Scraping Alcohol Stores ---");
    const alcoholStoreLinks = await scrapeVerticalLinks(page, "Alcohol", "https://www.doordash.com/vertical_homepage?cursor=&vertical_id=37", "alcoholStores", "alcohol");
    for (const store of alcoholStoreLinks) {
      await scrapeStoreProducts(page, store, "alcoholProducts");
    }
